...
## 3. Card Values
Points are calculated based on the cards remaining in a player's hand:
- **Joker:** 0 points
- **Ace:** 1 point
- **2 to 9:** Face value (e.g., a 5 is worth 5 points)
- **10, Jack, Queen, King:** 10 points each

---

## 4. Gameplay Flow
Unlike standard card games where you draw first and discard last, this variation uses the **Discard First, Draw Next** mechanics.

### **On Your Turn:**
1. **Discard Phase (First):**
   - You must select and discard at least one card from your hand onto the Discard Pile.
   - You can discard multiple cards in a single turn **only if they are of the exact same rank** (e.g., discarding two 8s or three Kings).
2. **Draw Phase (Next):**
   - You must draw exactly one card (either from the face-down Draw Pile or the face-up Discard Pile), *unless* the Skip Draw rule applies.
3. **Skip Draw Rule:**
...
## 5. Declaring a "Show" (Ending the Round)
If you believe you have the lowest total score in your hand, you can declare a **"Show"**.
...
## 6. Scoring & Penalties
At the end of a round, points are distributed as follows:
